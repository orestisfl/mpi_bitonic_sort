...
for Q, value in groupby(results, lambda x: x["Q"]):
    per_q[Q] = list(value)
...
value.sort(key=lambda x: x["ppn"])
for element in value:
    plots[element["nodes"]].append(element["bsort"]/element["qsort"])

x = np.array([1,2,4,8,16,32])
xticks = np.array([1,2,4,8,16,32])

sub_plot=0
m=0
for nodes, y in plots.items():
    if nodes==4:
        #~ break
        x = np.array([1,8,16,32])
        
    ax.plot(x, y, color=colors[sub_plot], linewidth=5, label=str(nodes))
    if max(y) > m:
        m=max(y)
    sub_plot+=1
    
ax.set_ylim(ymin=0, ymax=m*1.13)
ax.set_title("bsort/qsort for Q=%d" % Q)
ax.grid(aa=True)
ax.set_ylabel("bitonic parallel / qsort serial")
ax.set_xlabel("peers per node")
ax.xaxis.set_ticks(xticks)
handles, labels = ax.get_legend_handles_labels()
tmp = sorted(zip(labels, handles), key=lambda x: int(x[0]))
labels = [x for x,y in tmp]
...
fig.set_size_inches(20, 20)
fig.savefig('../plot_constQ.png',dpi=100)
